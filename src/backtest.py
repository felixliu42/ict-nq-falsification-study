...
# ---------------------------------------------------------------------------
# Per-DataFrame lookup cache for fast repeated trade simulation.
# Semantics are identical to the previous per-call scans: exact timestamp
# match takes the FIRST occurrence, and the fallback picks the closest bar
# within 1 hour. This only avoids re-scanning the full raw series and using
# slow scalar .loc access for every simulated trade.
# ---------------------------------------------------------------------------
_RAW_CACHE = {}

def _get_raw_arrays(df_raw):
    key = id(df_raw)
    cached = _RAW_CACHE.get(key)
    if cached is not None and cached['n'] == len(df_raw):
        return cached
    time_arr = pd.to_numeric(df_raw['time'], errors='coerce').to_numpy(dtype='float64')
    cached = {
        'n': len(df_raw),
        'time': time_arr,
        'high': pd.to_numeric(df_raw['high'], errors='coerce').to_numpy(dtype='float64'),
        'low': pd.to_numeric(df_raw['low'], errors='coerce').to_numpy(dtype='float64'),
        'close': pd.to_numeric(df_raw['close'], errors='coerce').to_numpy(dtype='float64'),
        'time_to_idx': {},
    }
    t2i = cached['time_to_idx']
    for i, tv in enumerate(time_arr):
        if tv == tv:  # skip NaN
            tv_int = int(tv)
            if tv_int not in t2i:  # first occurrence wins
                t2i[tv_int] = i
    _RAW_CACHE[key] = cached
    return cached

def _find_entry_idx(cached, entry_time):
    """
    Positional index of the entry bar: exact time match first, otherwise
    fall back to the closest bar within 1 hour (same rule as before).
    """
    idx = cached['time_to_idx'].get(int(entry_time))
    if idx is not None:
        return idx
    time_diffs = np.abs(cached['time'] - entry_time)
...
def simulate_trade_execution(row, df_raw, tp_mode='tp1'):
    ...
    # Find setup bar index in raw data (cached O(1) lookup)
    raw = _get_raw_arrays(df_raw)
    high_arr, low_arr, close_arr = raw['high'], raw['low'], raw['close']
    idx = _find_entry_idx(raw, entry_time)
    if idx is None:
    ...
    if sweep_dir == -1: # Bearish Setup
        sweep_extreme = float(high_arr[sweep_idx])
    else: # Bullish Setup
        sweep_extreme = float(low_arr[sweep_idx])
...
def get_trade_R(row, df_raw):
    ...
    entry_price = float(raw['close'][idx])
    sweep_idx = idx - time_since
    if sweep_idx < 0:
        return None
    ...
    # Get sweep extreme price (acts as Stop Loss)
    if sweep_dir == -1: # Bearish Setup
        sweep_extreme = float(raw['high'][sweep_idx])
    else: # Bullish Setup
        sweep_extreme = float(raw['low'][sweep_idx])
