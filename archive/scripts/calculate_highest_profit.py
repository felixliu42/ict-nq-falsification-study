...
def simulate_dollar_backtest(df_oos, df_raw, tp_mode, threshold, multiplier):
    """
    Simulate trades and return dollar-based performance metrics.
    """
    trades_df = df_oos[df_oos['pred_prob'] > threshold].copy()
...
def main():
    ...
    strategies = [
        ('Baseline (All Sessions)', baseline_oos),
        ('Variant 1 (Morning-Only + HTF Sweep Bias)', v1_oos),
        ('Variant 2 (Morning-Only + Stop Mult 0.90)', v2_oos),
        ('Variant 3 (Stacked Liquidity + Max 1 Setup)', v3_oos)
    ]
        ...
        for name, df_oos in strategies:
            ...
            for mode in modes:
                for th in thresholds:
                    res = simulate_dollar_backtest(df_oos, df_raw, mode, th, mult)
                    if res['trades'] > 0:
                        all_results.append({
                            'multiplier': mult,
                            'strategy': name,
    ...
    print("\n" + "="*145)
    print("TOP CONFIGURATIONS FOR 16 CONTRACTS ($32/point multiplier)")
    print("="*145)
    df_32 = df_results[df_results['multiplier'] == 32.0].sort_values('final_balance', ascending=False).reset_index(drop=True)
    print(f"| Rank | {'Strategy Name':<42} | {'TP Mode':<13} | {'Thresh':<6} | {'Trades':<6} | {'Win Rate':<8} | {'Exp (R)':<8} | {'Net Profit':<12} | {'Final Balance':<14} | {'Max DD ($)':<12} | {'Profit/DD':<9} |")
    print(f"|------|--------------------------------------------|---------------|--------|--------|----------|---------|--------------|---------------|--------------|-----------|")
    for i, r in df_32.head(20).iterrows():
        print(f"| {i+1:<4} | {r['strategy']:<42} | {r['mode']:<13} | {r['threshold']:<6.2f} | {r['trades']:<6} | {r['win_rate']:<8.1%} | {r['expectancy_r']:<8.2f} | ${r['net_profit']:<11,.2f} | ${r['final_balance']:<12,.2f} | ${r['max_dd']:<11,.2f} | {r['profit_to_dd']:<9.2f} |")
        
    # Sort and display top configurations for Multiplier = 16 ($16/point multiplier)
    print("\n" + "="*145)
    print("TOP CONFIGURATIONS FOR $16/POINT MULTIPLIER (8 contracts)")
    print("="*145)
